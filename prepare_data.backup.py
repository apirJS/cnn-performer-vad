...
def create_speech_gap_speech(
    ...
    # Generate labels for first speech segment using librosa.feature.rms
    frame_energies1 = librosa.feature.rms(
        y=first_speech, frame_length=win_length, hop_length=hop_length
    )[0]
    
    # Use the actual length from librosa instead of calculating it
    first_frames = len(frame_energies1)
    
    # Generate labels for gap (all zeros)
    # Calculate frame count for gap using librosa to ensure consistency
    frame_energies_gap = librosa.feature.rms(
        y=gap, frame_length=win_length, hop_length=hop_length
    )[0]
    gap_frames = len(frame_energies_gap)
    ...
    second_frames = len(frame_energies2)

    # Pre-allocate arrays with the correct sizes from actual calculations
    ...
    # Calculate threshold for second segment
